...
def SSHAnalysis(pkt):       #analyse les paquets ssh
    ...
    if pkt[IP].len == 130-14 or  pkt[IP].len == 150-14:  #paquets correspondant aux aux interactions de login
    ...
    if SSHattempts[key][port] > 4 and SSHsuccess[key][port] == 0:  # eventuelle tentative de brute force si + de 10 tentatives de connexion ET s'il n'y a pas eu de réussite
        print("Potential brute forcing detected. \n")

    if pkt[IP].len == 142-14 or pkt[IP].len == 102-14:
        print("\t\t..........DISCONNECT...........")
        SSHsuccess[key][port]=0
    ...
    with open("listeIP.txt", "a") as file:  # dump les IPS dans un JSON
        ...
        if pkt[IP].len == 87 - 14:
            ...
            json.dump({"src": src, "dst": dst}, file)
            son.dump({"warning": warning3}, file)
        file.write("\n")
